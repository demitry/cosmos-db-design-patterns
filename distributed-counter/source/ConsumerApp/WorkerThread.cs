...
namespace DistributedCounterConsumerApp
{
    ...
    internal class WorkerThread
    {
        ...
        public WorkerThread(PrimaryCounter _pc, DistributedCounterOperationalService _dcos,
            PostMessageCallback _postMessage)
        {
            this.pc = _pc;
            this.dcos = _dcos;
        ...
        public async void StartThread()
        {
            while (this.isActive)
            {
                ...
                try
                {
                    ...
                    {
                        postMessage(
                            $"[yellow bold]Failed[/]\t\t[italic strikethrough]Attemped to decrement by {decrementVal}[/]");
                    }
                    else
                    {
