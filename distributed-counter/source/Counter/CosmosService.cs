...
namespace CosmosDistributedCounter.Services;
...
public class CosmosService
{
    ...
    public async Task<PrimaryCounter> ReadPrimaryCounterAsync(string counterId)
    {
        try
        {
            return await container.ReadItemAsync<PrimaryCounter>(id: counterId,
    ...
    public async Task<PrimaryCounter> UpdatedPrimaryCounterStatusAsync(PrimaryCounter pc, CounterStatus status)
    {
        try
        {
            ...
            return await container.PatchItemAsync<PrimaryCounter>(pc.Id, new PartitionKey(pc.Id),
                patchOperations: operations,
                requestOptions: new PatchItemRequestOptions
                    { FilterPredicate = "FROM Counters c where c.status !=" + (int)status });
        }
    ...
    public async Task<List<DistributedCounter>> GetDistributedCountersAsync(string counterId, bool onlyActive = true,
        SortingOrder sorting = SortingOrder.NoSort)
    {
    ...
    public async Task<DistributedCounter> UpdatedDistributedCounterStatusAsync(DistributedCounter dc,
        CounterStatus status)
    {
        try
        {
            ...
            return await container.PatchItemAsync<DistributedCounter>(dc.Id, new PartitionKey(dc.ParentCounter),
                patchOperations: operations,
                requestOptions: new PatchItemRequestOptions
                    { FilterPredicate = "FROM Counters c WHERE c.status !=" + (int)status });
        }
    ...
    public async Task<DistributedCounter> UpdateDistributedCounterValueandStatusAsync(DistributedCounter dc,
        CounterStatus status, long value)
    {
        try
        {
            List<PatchOperation> operations = new()
            ...
            return await container.PatchItemAsync<DistributedCounter>(dc.Id, new PartitionKey(dc.ParentCounter),
                patchOperations: operations, requestOptions: new PatchItemRequestOptions { IfMatchEtag = dc.ETag });
        }
        catch (CosmosException e)
        {
            if (e.StatusCode == HttpStatusCode.PreconditionFailed)
    ...
    public async Task<DistributedCounter> DecrementActiveDistributedCounterValueAsync(string pcId, string dcId, long value)
    {
        ...
        return await container.PatchItemAsync<DistributedCounter>(dcId, new PartitionKey(pcId),
            patchOperations: operations,
            requestOptions: new PatchItemRequestOptions
                { FilterPredicate = "FROM Counters c WHERE c.status = 0 and c.countervalue >=" + value });
    }
}