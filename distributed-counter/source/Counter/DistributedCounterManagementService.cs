using CosmosDistributedCounter.Services;
...
public class DistributedCounterManagementService
{
    ...
    public DistributedCounterManagementService(string CosmosUri, string CosmosKey, string CosmosDatabase,
        string CosmosContainer)
    {
        cosmos = new CosmosService(CosmosUri, CosmosKey, CosmosDatabase, CosmosContainer);
    }

    public async Task<PrimaryCounter> CreateCounterAsync(string counterName, long initialValue,
        int distributedCounters)
    {
    ...
    public async Task<List<DistributedCounter>> GetDistributedCountersAsync(PrimaryCounter pc,
    ...
    {
        List<DistributedCounter> dcList =
            await cosmos.GetDistributedCountersAsync(pc.Id, true, CosmosService.SortingOrder.SortDescending);
        ...
        foreach (DistributedCounter dc in dcList)
        {
            if (ctr <= count)
            {
                ...
                DistributedCounter dc_updated =
                    await cosmos.UpdatedDistributedCounterStatusAsync(dc, CounterStatus.paused);

                long orgValue = dc.Value;
                long newValue = orgValue / 2;

                //update DC to 50% value, status =active, back to rotation
                await cosmos.UpdateDistributedCounterValueandStatusAsync(dc_updated, CounterStatus.active,
                    newValue);

                try
                {
                    //create new DC with 50% value
                    DistributedCounter dc_new =
                        await cosmos.CreateDistributedCounterAsync(pc.Id, orgValue - newValue);
                    ctr++;
                }
                ...
                catch (CosmosException ex)
                {
                    ...
                    {
                        if (await cosmos.UpdateDistributedCounterValueandStatusAsync(dc_updated,
                                CounterStatus.active, orgValue) != null)
                            break;
    ...
    {
        List<DistributedCounter> dcList =
            await cosmos.GetDistributedCountersAsync(pc.Id, true, CosmosService.SortingOrder.SortAscending);
            ...
            if (ctr <= count)
            {
                ...
                if (mergeWithNext == false)
                {
                    if (i < dcList.Count - 1) //ensure there is next item to merge
                    {
                        ...
                        {
                            dc_deleted = dc;
                            dc_merged = await cosmos.UpdatedDistributedCounterStatusAsync(dc,
                                CounterStatus.deleted);
                ...
                else
                {
                    for (int j = 0; j < 5; j++) //try 5 times
                    {
                        ...
                        if (j == 5)
                        {
                            ...
                            {
                                dc_deleted =
                                    await cosmos.UpdatedDistributedCounterStatusAsync(dc_deleted,
                                        CounterStatus.active); //restore back to active
                            }
                            catch
                            {
                                throw new Exception("DC Merge failed into " + dc.Id + " , source was " +
                                                    dc_deleted.Id); //if merge failed and also restore
                            }
                        }
                    }
